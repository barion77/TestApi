...
class Wallet extends Freekassa
{
     ...
     * @return array $result
     */
    public function payment($shod_id, $nonce, $i, $email, $ip, $amount, $currency, $payment_id = NULL, $success_url = NULL, $failure_url = NULL)
    {
        try {
            $result = $this->request('orders/create', compact('shod_id', 'nonce', 'i', 'email', 'ip', 'amount', 'currency', 'payment_id', 'success_url', 'failure_url'));
        } catch (\Exception $e) {
            return $e->getMessage();
        }
